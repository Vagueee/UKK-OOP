...
def start_paket(self, role):
    ...
    menubar = tk.Menu(self.paket)
    self.paket.config(menu=menubar)

    menu = tk.Menu(menubar, tearoff=0)
    menubar.add_cascade(label="Menu", menu=menu)
    menu.add_command(
        label="Exit", command=self.paket.destroy)

    self.canvas.create_text(480, 50, text="Data Paket", anchor="center", font=(
